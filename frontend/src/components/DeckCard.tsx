...
function formatLastAccessedDate(date: Date): string {
  const now = new Date();
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between border-t border-white/5 pt-4">
        ...
        <span className="text-xs leading-4 text-neutral-300/70">{`Last studied: ${formatLastAccessedDate(deck.lastAccessed)}`}</span>
      </div>
    </div>
  );
}